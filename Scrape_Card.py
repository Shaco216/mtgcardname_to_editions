...
def cardname_with_space(cardname,url):
    url_lenght = len(url)
    laenge_cardname = len(cardname) + 2
    laenge_diff = url_lenght - laenge_cardname
    diff = url_lenght - 6
    url_cardname = url[laenge_diff:diff]
    #print(url_cardname)
    url_cardname.replace('+', '-')
    return url_cardname
...
def get_card_information(url, cardname, filetype):
    ...
    if filetype == 'TXT':
        ...
        cardname = 'Kartenname: ' + cardname + '\t\t' + 'Expansions: '
    ...
    if filetype == 'TXT' or filetype == 'txt' or filetype == 'Txt':
        ...
        for item in Expansionlist:
            ...
            Explist.append(temp)
        print(Explist)
    else: # hier wird für csv die explist erstellt
        for item in Expansionlist:
            temp = str(item)
            temp = temp.replace('/de/Magic/Expansions/', '')
            Explist.append(temp)
...
if __name__ == '__main__':
    ...
    cardname_with_space(cardname,url)
